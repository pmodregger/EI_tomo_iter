...
def radon_numba_linear(data,theta,RA,func=0): # radon transform
    """
    USAGE:
        out = radon_numba_linear(data,theta,RA,func=0,1,2)

        theta = in radians
        RA         

        func = 0: radon transform
            data = [Ndet,Ndet] - slice
            out = [Ndet,Ntheta] - sino
        func = 1: grad radon transform
            data = [Ndet,Ntheta] - 'sino'
            out = [Ndet,Ndet] - 'slice'
        func = 2: grad diff radon transform
            data = [Ndet,Ntheta] - 'sino'
            out = [Ndet,Ndet] - 'slice'    
    """
    # print('old func',func)
    ...
    if func: # grad radon transform OR grad diff radon transform
    ...
    for isi in range(Ndet*Ndet):
        ...
        if a:
            for ist in range(Ntheta):
                ...
                if func==0: # radon transform
                    b = 1/(d1n+d2n)
                    out[tt,ist] += data[isy,isx]*b*d2n
                    out[tt+1,ist] += data[isy,isx]*b*d1n
                elif func==1: # grad radon transform
                    out[isy,isx] += (data[tt,ist]*d2n + data[tt+1,ist]*d1n)/(d1n+d2n)
                elif func==2: # grad diff radon transform
                    out[isy,isx] += (data[tt,ist]*d2n + data[tt+1,ist]*(d1n-d2n) - data[tt+2,ist]*d1n)/(d1n+d2n)
    return out
...
def iter_tomo_rec(expsino,iter_dict):
    ...
    def L(p):
        ...
        theta in rad
        """
        pradon = radon_numba_linear(np.reshape(p,((Ndet,Ndet))),theta,RA,0)
        # print(np.sum((expsino-pradon)**2)*C)
    ...
    def gradL(p):
        ...
        # print(grad_radon_numba_linear(temp,theta,RA).flatten()[0])
        return radon_numba_linear(temp,theta,RA,1).flatten()
...
def iter_single_shot(x,f,m,s,iter_dict):
    ...
    global Niter, slice_abs, mo, mr, flatm, Liter, gamma, icfac, RA
    Niter = 0
        ...
        if iter_dict['do_rec_slice_abs']:
            if iter_dict['do_rec_slice_phs']:
                ...
                return S(p)+lambda1*Huber(p[rec_slice_abs_inds],delta1)\
                           +lambdatv*TV(np.reshape(p[rec_slice_abs_inds],(Ndet,Ndet)))
        else:
            return S(p)
        ...
        if iter_dict['do_rec_slice_abs']:
            if iter_dict['do_rec_slice_phs']:
                ...
                return gradS_list_line2(p)\
                        ...
                        + np.append(lambdatv*gradTV(np.reshape(p[rec_slice_abs_inds],(Ndet,Ndet))),np.zeros(Nrecvalues-len(rec_slice_abs_inds)))
        else:
            return gradS_list_line2(p)
    
    def S(p): # cost function of model part (slice, mr, mo)
        global slice_abs, slice_phs, mo , mr, gamma, RA
        ...
        if iter_dict['do_rec_slice_abs']:
            fradon = radon_numba_linear(np.reshape(slice_abs,((Ndet,Ndet))),theta,RA,func=0)
    ...
    def gradLnum(p):
        ...
        L_ = L(p)
        for isn in range(len(p)):
            p0 = p.copy(); p0[isn] = p[isn]+1e-8
            gradLnum_[isn] = (L(p0)-L_)/1e-8
