...
version 5.5 fork into numba or openmp
version 5.4 is copy of 5.2
version 5.3 -> discarded
...
def iter_parameters():
    ...
    iter_dict['radon_algo'] = 'numba' # numba or openmp
...
def single_shot(x0,xs,ICflat,samp,theta,gamma,dorec=1):
    ...
    f0 = np.zeros((Nvoxel)) # flat IC at measure point m: f(m,t)
    fs = np.zeros((Nvoxel)) # derivative of flat IC at measure point m: f'(m) (from mean f(m,t))
    for isp in range(Nvoxel):
        f0[isp] = np.interp(x0,xs,ICflat[isp,:])
    fs =  np.interp(x0,xs,np.append(0,np.diff(np.mean(ICflat,0))))/Dxs

    c = fs/f0 # here it makes not much difference if f0 or mean(f0) is used
...
def iter_tomo_rec(expsino,iter_dict):
    ...
    def L(p):
        ...
        theta in rad
        """
        pradon = radon.radon_linear(np.reshape(p,((Ndet,Ndet))),theta,RA)
    ...
    def gradL(p):
        ...
        return radon.radon_linear_grad_0(temp,theta,RA).flatten()
...
def iter_single_shot(x,f,m,s,iter_dict):
    ...
    if iter_dict['radon_algo'] == 'numba':
        import radon_numba_lib as radon
        print('Using numba radon transform')
    elif iter_dict['radon_algo'] == 'openmp':
        import radon_omp_lib as radon
        print('Using openmp radon transform')
    else:
        raise NameError('Unknown radon method (use either numba or openmp)')
    ...
    def S(p): # cost function of model part (slice(s), mr, mo)
        ...
        if iter_dict['do_rec_slice_abs']:
            fradon = radon.radon_linear(np.reshape(p[rec_slice_abs_inds],((Ndet,Ndet))),theta,RA)
            Aradon = np.exp(-gamma_abs*fradon)
        else:
            Aradon = np.ones((Ndet,Ntheta))        
        if iter_dict['do_rec_slice_phs']:
            Dradon = np.zeros((Ndet,Ntheta))
            Dradon[1:,:] = gamma_phs*np.diff(radon.radon_linear(np.reshape(slice_phs,((Ndet,Ndet))),theta,RA,0),1,0)
        else: # single shot
            Dradon = np.zeros((Ndet,Ntheta))    
    ...
    def gradS_list_line2(p): # gradient of cost function of radon part 
        ...
        if iter_dict['do_rec_slice_abs']:
            ...
            if iter_dict['do_rec_slice_phs']: # do seperate absorption & phase reconstructions
                gradS_list_line[rec_slice_abs_inds] = radon.radon_linear(temp1,theta,RA,1).flatten()
                gradS_list_line[rec_slice_phs_inds] = radon.radon_linear(temp2,theta,RA,2).flatten()
            else: # single shot
                ...
                gradS_list_line[rec_slice_abs_inds] = radon.radon_linear_grad(temp1,temp2,theta,RA).flatten()
    ...
    if iter_dict['do_rec_mr']:
        rec_mr_inds = iter_dict['do_rec_slice_abs']*Ndet*Ndet+iter_dict['do_rec_slice_phs']*Ndet*Ndet+iter_dict['do_rec_mo']*M*Ntheta+np.arange(Ndet)
