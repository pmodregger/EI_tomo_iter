    ...
    for isi in range(Ndet*Ndet):
        ...
        if a:
            for ist in range(Ntheta):
                ...
                d1n = (t[ist]-tt)**1.5  # this here is an interpolation
                d2n = (tt+1-t[ist])**1.5
                ...
                # tt = np.int32(t[ist])
                # u = t[ist]-tt
                # d1n = u**1.5  # this here is an interpolation
                # d2n = (1-u)**1.5
                # b = 1/(d1n+d2n)
                # print(t[ist],tt,d1n,d2n,b)
    ...
    for isi in range(Ndet*Ndet):
        ...
        if a:
            for ist in range(Ntheta):
                ...
                tt = np.int32(t[ist])
                u = t[ist]-tt
                d1n = u**n  # this here is an interpolation
                d2n = (1-u)**n
...
def iter_single_shot(x,f,m,s,iter_dict):
    ...
    def S(p): # cost function of model part (slice(s), mr, mo)
        global Aradon, Dradon, fs # these are shared between S and gradS
