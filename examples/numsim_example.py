...
Updated for iterlib v 5.5

Example for numerical simulation for the iterative reconstruction. It should
take about 5 seconds to show the results. Script has to be run in the
...
Ntheta = Ndet # Ndet//2 # number of projections
...
mr_org = (0.5-np.random.rand(Ndet,M))*0 # artificial ring introduction
...
Mflat_factor = 20 # simulated number of flat scans
...
gamma_sim = 5 # gamma used in forward model

#%% defining the sample slice
# absorption slice
orga = phantom.phantom(Ndet,ellipses=[[1.2,0.4,0.4,0,-0.4,0],
                                  [1.0,0.3,0.3,-.4,0.4,0],
                                  [0.8,0.3,0.3,.4,0.4,0]])*2**2/Ndet/10*2*2*1
# phase slice
...
asino = radon.radon_linear(orga,theta,RA);
asino = np.exp(-gamma_abs*asino) # absorption contrast sino
tsino = radon.radon_linear(orgp,theta,RA);
...
if flatnoiseon: # add photon shot noise if turned on
    ICflat, dummy = countstat(Mflat_factor*ICflat)
    ICflat = ICflat/Mflat_factor
...
iter_dict['do_rec_mr'] = 0 # ring artefact
